int whitepaperplot()
{
  ...
  TString path_to_sims = "/sphenix/user/gregtom3/ephenix-sbu/analysis/gregory_matousek/vlad_dvmp_but_fast/condor_sartre_smear_whitepaper/100000000-1_10x110_T_-0.180_-0.000_Q2_1_10_XPOM_0_1/";
  ...
  for(int i = 0 ; i < 8 ; i++)
    {
      ...
      //      t[i]=(TTree*)f[i]->Get("event");
      h[i][0]=(TH1F*)f[i]->Get(Form("%s_%d",hnames[i][0].Data(),tsave));
      h[i][1]=(TH1F*)f[i]->Get(Form("%s_%d",hnames[i][1].Data(),tsave));
      //      h[i][1]=(TH1F*)f[i]->Get(Form("h_t_incoherent_smearing_%d",tsave));
      //      h[i][0]->SetName(hnames[i][0]);
      //      h[i][1]->SetName(hnames[i][1]);
      ...
      //      t[i]->SetBranchAddress("true_crossSection",&xsec_temp);
      //      t[i]->GetEntry(1);
      //      xsec[i]=xsec_temp;
    }
  
  //  t[0]->Draw(Form("true_t>>%s",hnames[0][0].Data()));
  //  return 0;
  //  double dt = (tmax-tmin)/(1.0*bins);
  //  int A = 197;
  //  lumi=lumi*pow(10.,6)/(A*1.0); // lumi in nb^-1/A
  ...
  for(int i = 0 ; i < 8 ; i++)
      ...
      for(int j = 0 ; j < 2 ; j++)
	{
	  ...
	  tge[i][j]->GetYaxis()->SetRangeUser(0.001,300000.0);
  ...
  for(int i = 0 ; i < 2 ; i++)
    {
      ...
      if(i==0)
	{
	  pt[i][0] = new TPaveText(0.05,800,0.17,20000.0);
	  pt[i][1] = new TPaveText(0.17,100,0.24,20000.0);
	  
	}
      else
	{
	  pt[i][0] = new TPaveText(0.05,8000,0.17,200000.0);
	  pt[i][1] = new TPaveText(0.17,1000,0.24,200000.0);
	}
      ...
      pt[i][1]->AddText("y > 0.01");
      if(i==0)
	{
	  pt[0][1]->AddText("|#eta(e_{#lower[-0.4]{decay}})| < 4");
	  pt[0][1]->AddText("p(e_{#lower[-0.4]{decay}}) > 1 GeV/c");
	}
      else
	{
	  pt[1][1]->AddText("|#eta(K_{#lower[-0.4]{decay}})| < 4");
	  pt[1][1]->AddText("p(K_{#lower[-0.4]{decay}}) > 1 GeV/c");
	}
      ...
      else
	{
      ...
      if(i==0)
	{
	  tge[0][0]->GetYaxis()->SetTitle("d#sigma^{(e+Au->e'+Au'+J/#Psi)}/dt (nb/GeV^{2})");
	  tge[0][0]->GetYaxis()->SetRangeUser(0.001,30000.0);
	  tge[0][0]->SetTitle("");
	...
	{
	  tge[3][0]->GetYaxis()->SetTitle("d#sigma^{(e+Au->e'+Au'+#phi)}/dt (nb/GeV^{2})");
	  tge[3][0]->SetTitle("");
